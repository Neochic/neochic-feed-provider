...
class FeedProvider {
    ...
    public function getFacebookFeed($fbProfileId, $appId, $appSecret, $amount = 3) {
        $cacheFile = $this->getTmpDir() . '/ncfp_facebook_feed_' . $appId . '.json';
        if (is_file($cacheFile)) {
            $data = json_decode($this->fileGetContentsCurl($cacheFile));
        ...
        try {
            ...
            $facebookFeed = json_decode($this->fileGetContentsCurl($url));
    ...
    protected function fileGetContentsCurl($url, $curlopt = array())
    {
        $ch = curl_init();
    ...
    protected function attachToWoodlets() {
        if ($this->withinWordpressWoodletsContext()) {
            add_filter('neochic_woodlets_twig', function($twig) {
                ...
                $twig->addFunction(new \Twig_SimpleFunction('getFacebookFeed', function ($fbProfileId, $appId, $appSecret, $amount = 3) {
                    return $this->getFacebookFeed($fbProfileId, $appId, $appSecret, $amount);
                }));
                return $twig;
            });
        }
